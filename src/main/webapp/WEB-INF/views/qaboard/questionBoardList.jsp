...
<%--                <th scope="col" class="px-6 py-3">본문</th>--%>
            ...
            <c:forEach items="${questionBoards}" var="qaboard" varStatus="vs">
                <tr class="odd:bg-white even:bg-gray-50 border-b ">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">${qaboard.qbNo}</th>
                    <td class="px-6 py-4">${qaboard.memberNo}</td>
                            <%--  조회수인듯 몰겠음 일단 주석함  --%>
...
<%--                        </c:if>--%>
                    <td class="px-6 py-4">
                        <a href="${pageContext.request.contextPath}/qaboard/questionBoardDetail?qaNo=${qaboard.qbNo}" class="hover:underline">${fn:escapeXml(qaboard.title)}</a>
                    </td>
                    <td class="px-6 py-4">${qaboard.regDate}</td>
<%--                    <td class="px-6 py-4">--%>
<%--                        <fmt:parseDate value="${qaboard.regDate}" pattern="yyyy-MM-dd'T'HH:mm" var="regDate"/>--%>
<%--                        <fmt:formatDate value="${regDate}" pattern="yy/MM/dd"/>--%>
<%--                    </td>--%>
                    <td class="px-6 py-4">${qaboard.adminChoice}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
